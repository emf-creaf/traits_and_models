...
#
harmonize_Baez_et_al_2022_FUNANDES <- function(DB_path = "./", checkVersion = as.character(packageVersion("traits4models"))) {
  ...
  db_var <- fun_db |>
    dplyr::select("SpeciesName", "OrigValueStr", "OriginalName", "OrigUnitStr", "Dataset", "Plot_ID", "Plant_ID") |>
  ...
  db_var <- db_var |>
    dplyr::mutate(Level = "taxon",
                  Level = ifelse(is.na(Plant_ID) & !is.na(Plot_ID), "population", Level),
                  Level = ifelse(!is.na(Plant_ID) & !is.na(Plot_ID), "individual", Level))  |>
    dplyr::relocate(Level, .after = Units) |>
    dplyr::select(-Plot_ID, -Plant_ID)
  ...
  # Bark thickness --------------------------------------------------
  db_var <- fun_db |>
    dplyr::select("SpeciesName", "OrigValueStr", "OriginalName", "OrigUnitStr", "Plot_ID", "Plant_ID") |>
  ...
  # LDMC --------------------------------------------------
  db_var <- fun_db |>
    dplyr::select("SpeciesName", "OrigValueStr", "OriginalName", "Plant_ID", "Plot_ID") |>
  ...
  # SLA --------------------------------------------------
  db_var <- fun_db |>
    dplyr::select("SpeciesName", "OrigValueStr", "OriginalName", "OrigUnitStr", "Plant_ID", "Plot_ID") |>
