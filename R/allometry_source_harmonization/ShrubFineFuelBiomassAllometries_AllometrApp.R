harmonize_ShrubFineFuelBiomassAllometries_AllometrApp <- function(DB_path = "./", checkVersion = as.character(packageVersion("traits4models"))) {
  WFO_file <- paste0(DB_path, "data-raw/wfo_backbone/classification.csv")
  
  # Shrub fine fuel (from AllometrApp) ---------------------------------------
  allom_db  <- readxl::read_xlsx("data-raw/raw_allometry_data/AllometrApp/shrub_fine_fuel_allometries_catalonia_2024-09-05.xlsx")
  var_db <- allom_db[1:27,] |>
    ...
    dplyr::rename(originalName = functional_group_level_name,
                  ...
                  a = "param_a",
                  b = "param_b") |>
    dplyr::mutate(Equation = "FineFuelBiomass = a·PHV^b",
                  Response = "FineFuelBiomass",
                  ResponseDescription = "Fine fuel biomass (kg)",
                  Predictor1 = "PHV",
  ...
  write.csv2(db_post, "data/harmonized_allometry_sources/shrub_fine_fuel_biomass_catalonia.csv", row.names = FALSE)
}
